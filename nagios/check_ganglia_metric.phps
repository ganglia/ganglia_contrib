...
# checks this may be quite heavy so you may want to cache data. Default gmetad polling
# interval is 15 seconds so you would be well advised to utilize caching
define("NAGIOSCACHEDATA", 1);
define("NAGIOSCACHEFILE", 	"/tmp/nagios/ganglia_metrics");
define("NAGIOSCACHETIME",	45); // How long to cache the data in seconds
...
if(NAGIOSCACHEDATA == 1 && file_exists(NAGIOSCACHEFILE)){
    	// check for the cached file
    	// snag it and return it if it is still fresh
	$time_diff = time() - filemtime(NAGIOSCACHEFILE);
	$expires_in = NAGIOSCACHETIME - $time_diff;
     	if( $time_diff < NAGIOSCACHETIME){
		if ( $debug == 1 ) {
		  echo("DEBUG: Fetching data from cache. Expires in " . $expires_in . " seconds.\n");
		}
     		$metrics = unserialize(file_get_contents(NAGIOSCACHEFILE));
     	}
}
...
if ( ! is_array( $metrics ) ) {
  ...
  file_put_contents(NAGIOSCACHEFILE, serialize($metrics));
